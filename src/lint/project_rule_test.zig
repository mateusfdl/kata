...
const lint_diagnostic = @import("core").diagnostic;
const project_rule = @import("core").ProjectRule;
...
test "project rule: restricted-callers flags non-repository callers only" {
    ...
    const violations = try project_rule.evaluate(arena_state.allocator(), &.{repository_isolation}, &.{}, &index, null);
...
test "project rule: path filter keeps cross-file context but reports one file" {
    ...
    // The callee type is defined in another file; filtering must still see it.
    const filtered = try project_rule.evaluate(arena_state.allocator(), &.{repository_isolation}, &.{}, &index, "src/order-service.ts");
    try std.testing.expectEqual(@as(usize, 1), filtered.len);
    try std.testing.expectEqualStrings("src/order-service.ts", filtered[0].path);

    const clean = try project_rule.evaluate(arena_state.allocator(), &.{repository_isolation}, &.{}, &index, "src/audit-repository.ts");
...
test "project rule: import-boundary resolves ts relative specifiers" {
    ...
    const violations = try project_rule.evaluate(arena_state.allocator(), &.{domain_no_infra}, &.{}, &index, null);
...
test "project rule: import-boundary matches go import strings" {
    ...
    const violations = try project_rule.evaluate(arena_state.allocator(), &.{rule}, &.{}, &index, null);
