...
fn lowerSrc(arena: std.mem.Allocator, src: []const u8) lower.Error!lower.Lowered {
    ...
    var lowerer = lower.Lowerer.init(arena, language.grammar(.ts));
...
test "lower: field pattern assigns capture ids by occurrence" {
    ...
    try std.testing.expectEqualStrings("variable_declarator", result.pattern.kind.symbol);
    ...
    try std.testing.expectEqualStrings("identifier", field.pattern.kind.symbol);
...
test "lower: alternation lowers each branch" {
    ...
    try std.testing.expectEqualStrings("function_declaration", branches[0].kind.symbol);
    try std.testing.expectEqualStrings("arrow_function", branches[1].kind.symbol);
    try std.testing.expectEqual(@as(?query.CaptureId, 0), result.pattern.capture);
}
...
test "lower: unknown node kind fails with the offending name" {
    ...
    var lowerer = lower.Lowerer.init(arena.allocator(), language.grammar(.ts));
